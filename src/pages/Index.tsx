        ...
        <div className="">
            ...
            <div className="container m-auto py-5">
                <div className="flex justify-between items-center px-2">
                    <div className="text-4xl font-bold">Разделы</div>
                    <div className="text-gray-600 text-sm">Всего разделов: {categories.data.length}</div>
                </div>
                <div className="bg-white rounded-xl mt-5 grid lg:grid-cols-4 md:grid-cols-3 sm:gap-10 sm:p-8 p-4 justify-items-center grid-cols-2 gap-2">
                    ...
                    {!categories.isLoading && categories.data.map((category: Category) => <CatalogCard key={category.categoryId} categoryId={category.categoryId} title={category.title} imageUrl={category.imageUrl} />)}
                </div>
            </div>
        </div>
    )
