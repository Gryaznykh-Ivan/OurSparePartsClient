...
export default function ProductCard({ productId, title, price, imageUrl, onAddToCart, onOrder }: Props) {
    return (
        <div className="w-full bg-white rounded-xl p-5 space-y-2 flex flex-col justify-between">
            <Link to={ `/product/${ productId }` }>
                <img className="object-contain w-full md:h-64" src={ imageUrl || noImage } alt="" />
            ...
            <div className="">
                ...
                <div className="flex space-x-2 mt-2">
                    <button className="flex-1 bg-green text-white text-center p-1 rounded-xl" onClick={ () => onOrder({productId, title, price}) }>Заказать</button>
                    <button className="w-8 h-8  rounded-xl flex items-center justify-center" onClick={ () => onAddToCart({productId, title, price}) }>
